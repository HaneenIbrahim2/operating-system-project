int copy_paste_chunk(uint32* page_directory, uint32 source_va, uint32 dest_va,
	...
	for (int i = 0; i < no_frames; i++) {
		ret = get_page_table(page_directory, dest_va + (i * PAGE_SIZE),
				&dst_page_table);
		if (ret == TABLE_IN_MEMORY) {
			entry_dst = dst_page_table[PTX(dest_va+(i*PAGE_SIZE))];
			if ((entry_dst & PERM_PRESENT) > 0
					&& (entry_dst & PERM_WRITEABLE) <= 0)
				return -1;
		}
	}
	...
	for (int i = 0; i < no_frames; i++) {
		...
		if (ret == TABLE_NOT_EXIST)
			dst_page_table = create_page_table(page_directory,
					dest_va + (i * PAGE_SIZE));
		
		dst_frame_ptr = get_frame_info(page_directory,
				dest_va + (i * PAGE_SIZE), &dst_page_table);
		src_frame_ptr = get_frame_info(page_directory,
				source_va + (i * PAGE_SIZE), &src_page_table);
		
		entry_src = src_page_table[PTX(source_va+(i*PAGE_SIZE))];
		
		if (dst_frame_ptr == NULL) {
			allocate_frame(&dst_frame_ptr);
			map_frame(page_directory, dst_frame_ptr, dest_va + (i * PAGE_SIZE),
					PERM_WRITEABLE | (PERM_USER & entry_src));
		}
